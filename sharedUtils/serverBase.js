  ...
  findGame (player) {
    ...
    for (var gameid in this.games) {
      ...
      if(game.player_count < game.players_threshold) {
        ...
        // Add game to player
        player.game = game;
        player.role = game.playerRoleSequentialNames[game.player_count - 1];
        player.send('s.join.' + game.players.length + '.' + player.role);

        this.log('[DEBUG] current we have ' + game.players.length + " in the game of joining.");
        ...
        // adding a logic here to check whether we are actually
        // meeting the number of players requirements
        if (game.player_count == game.players_threshold) {
          // Start game
          this.startGame(game);
        } else {
          // send notification for drawing
